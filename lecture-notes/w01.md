...
<details>
    <summary>So, what are some use cases for references?</summary>
...
3. **Operator Overloading**
    ...
    what allows you to chain multiple output operations together.

        1. `(std::cout << x)` executes and returns a reference to `std::cout`.
        2. The next operation is then `(returned_stream_reference << y)`.

    Without returning a reference, this chaining would be impossible or would
    require inefficient copying of the stream object.
</details>
