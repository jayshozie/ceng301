...
<a href="../slides/w01.pdf">
    <img
        ...
        alt="A representation of a pill with one side reading Data, the other reading Functionality."
        style="width:35%;
        height:auto;">
</a>
<br/><br/>
...
The protection level determines the access that client code (any code outside
of the class itself) has to the member data or functionality.
- **Public** members can be accessed by client code.
- **Private** members can *only* be used within the class itself. Thus, client
code *cannot* access private members.
...
public:
    ...
    // lines above are called constructors, they initialize the object.
...
In C++, an object is declared exactly like a primitive type variable.
```cpp
#include <iostream>
#include "IntCell.h"
...
int main()
{
    // defined and correct declarations
    IntCell m1;
    IntCell m2(8);
    IntCell *m3;
...
- **new** automatically creates an object with the proper size, calls the class
constructor and returns a pointer of the correct type.
- **delete** destroys the object and frees space.
- They're used in a similar way to `malloc` and `free` in C.

### Syntax

```cpp
TypeName *typeNamePrt;
...
// dynamically allocate an array of 23 IntCell slots
...
By default, every variable in C++ is place din the *stack memory*. Stack memory
is associated with the current function and the memory's lifecycle is tied to
the function. When the function returns or ends, the stack memory of that
function is released. The stack memory always starts from high addresses and
grows down:

<a href="../slides/w01.pdf">
    <img
        src="./images/stack-memory-1.png"
        alt="A representation of the stack memory with pseudo-addresses at the left hand side."
        style="width:25%;
        height:auto;">
</a>
<br/><br/>

For example, take a look at the program below. What do you think the output
will be like?
```cpp
#include <iostream>
...
```
<details>
    <summary> The output of the above program will be something like this: </summary>

```console
num in main(): 7
&num in main(): 0x7ffc58082c84
x in foo(): 42
&x in foo(): 0x7ffc58082c64
```
</details>
...
As you can remember, the stack memory is only valid as long as the lifecycle of
the function. If we need a memory that is longer than that, then we need to use
the *heap memory*.

The _only_ way to create heap memory in C++ is with the **new** operator.

The **new** operator returns a pointer to the memory storing the data, **not**
an instance of the data itself.

## C++'s `new` Operator in Detail
...
The memory is only ever reclaimed by the system when the pointer is passed to
the **delete** operator, obviously if the program ends the systems reclaims
that memory, but while the same program is executed, system can't reclaim that
memory if the pointer is not passed to the **delete** operator.
...
## Stack and Heap Memory Used in Example
...
<a href="../slides/w01.pdf">
    <img
        src="./images/stack-heap-memory-1.png"
        alt="A representation of the stack memory with pseudo-addresses at the left hand side."
        style="width:25%;
        height:auto;">
</a>
...
<a href="../slides/w01.pdf">
    <img
        src="./images/stack-heap-memory-2.png"
        alt="A representation of the stack memory with pseudo-addresses at the left hand side."
        style="width:25%;
        height:auto;">
</a>
...
In C++, functions can have default arguments. This is specified inn the
function declaration.
```cpp
int foo(int x=1, int y=2, int z=3);

foo();      // all parameters use the default value
foo(5);     // y and z use the default value, x is 5
foo(5,8);   // z uses the default value, x is 5, y is 8
foo(5,8,9); // default values are not used, x is 5, y is 8, z is 9
```

Note that it's impossible to supply a user-defined value for z without also
supplying a value for x and y. That is, the following line would give a compile
error:
```cpp
foo(,,9);
```
The output of that compile process would look something like this:
```bash
❯ test
Compiling...
testBench.cpp: In function ‘int main()’:
testBench.cpp:7:9: error: expected primary-expression before ‘,’ token
    7 |     foo(,,9);
      |         ^
testBench.cpp:7:10: error: expected primary-expression before ‘,’ token
    7 |     foo(,,9);
      |          ^
```
...
```cpp
int foo(int x=1, int y=2, int z); // WRONG USE
int foo(int z, int x=1, int y=2); // CORRECT USE
```
