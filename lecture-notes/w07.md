...
**recursively**! Common orderings for traversals include:
    - **Pre-Order:** Visit the *current* node, visit the *left* subtree, then
...
Pretty neat trick to quickly see which traversal an algorithm is using.
...
void printSideways(IntTreeNode* root, std::string indent) {
    this->printSideways(root->right, indent + "   ");
    std::cout << indent << root->data << std::endl;
    this->printSideways(root->left, indent + "   ");
}
```
</details>
...
- a root node `R` such that:
    - every element of `R`'s left subtree contains data **less than** `R`'s
    data,
...
> **Binary Tree:** a <u>tree</u> where nodes have no more than 2 children.
> **Binary Search Tree:** a <u>binary tree</u> with a special ordering property.
...
3b --> 3z2((_))
```
In the above diagram, you can see 3 binary trees with the same nodes but
different heights.

## Searching BSTs

The BSTs below contain the same elements, but which structure is better for
searching?
...
A **balanced tree** is a tree whose subtrees differ in height by at most 1 and
are themselves balanced. So, a balanced tree of $`n`$ nodes has a height of
approximately $`\log_2 {n}`$. A very unbalanced tree can have a height close to
$`n`$.

To explain more clearly, for a tree to be balanced, the difference in height
between the left and right subtrees of *any* node is at maximum 1. There are
[multiple types of balance](./extra-w07.md), but we won't get into them.
...
> [!NOTE]
> The runtime of adding/deleting to and searching in a BST is closely related
> to its height.
