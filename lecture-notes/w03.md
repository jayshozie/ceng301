...
\text{when i=n} \\
    ...
    &=2^n \cdot c_1 + (\sum_{i=1}^{n-1} 2^i)\cdot c, \text{Summation Identity: Finite Geometric Series } \sum_{i=1}^{k-1} x^i = \frac{x^k - 1}{x - 1} \\
    &=2^n \cdot c_1 + (2^n - 1)\cdot c = 2^n \cdot (c_1 + c) - c
\text{So, the growth rate function is } &O(2^n)
\end{align*}
```
