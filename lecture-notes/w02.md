...
> [!NOTE]
> The slideshow that this markdown is based on includes both weeks 2 and 3,
> however I decided that it should just be called week 2, because that both
> confuses the reader, and makes it harder for me to separate them.
...
3. *Templates* to allow the same code to be used on a variate of different data
types.
4. A clean *built-in I/O interface*, which itself involves overloading the
...
- `~IntCell();`
- `~Cube();`
It can't receive any parameters, and can't return any values. You can only have
...
different files. For small-scale projects it may not matter. However,
separating interface and implementation has several advantages:
- Changes to implementation do not require recompilation of all client code.
- Changes to implementation do not require redistribution of client code.
- Implementation files can be hidden from the client.
...
- ***Client Program File:*** The *client program file* contains the code that
uses a class. Usually ends with `.cpp`. `.cpp` file includes the `.h` file with
...
int main(){
    ...
    a = 20; // this line will raise an error
}
```
```bash
testBench.cpp: In function ‘int main()’:
testBench.cpp:6:7: error: assignment of read-only variable ‘a’
    6 |     a = 20; // this line will give an error
      |     ~~^~~~
```
You can use `const` with/for a lot more stuff. You can check the keyword out at
[cppreference.com](https://en.cppreference.com/w/cpp/keyword/const.html).

## Example Use Cases of Keyword `const` in Class `Complex`

We may encounter `const` in this course in the following cases:
...
> [!IMPORTANT]
> Templates need to be declared AND implemented in the file you include. You
> cannot separate template class declaration and implementation and then only
...
of a vector
- **The `pop_back()` Member Function:** Removes the last element of the
container
```cpp
vector<int> v;
v.push_back(3);
v.push_back(2);
// v[[0] = 3, v[1] = 2, and v.size() is 2
