...
template<class T> void Queue<T>::stutter()
{
    if(this->isEmpty()) {
        std::stringstream errStr;
        errStr << "Error: Empty queue, cannot stutter." << std::endl;
        throw QException(errStr);
    }
    else {
        Queue<T> backup;
        T stutter;
        while(!this->isEmpty()) {
            backup.enqueue(this->peek());
            stutter = this->dequeue();
            backup.enqueue(stutter);
        }
        *this = backup;
    }
}
