...
MultiDimensionalPoint::MultiDimensionalPoint(const MultiDimensionalPoint& source)
...
MultiDimensionalPoint& MultiDimensionalPoint::operator=(const MultiDimensionalPoint& rhs) {
    // self-assignment check
    if (this != &rhs) {
        double* tmp = new double[rhs.dimension];

        for (int i = 0; i < rhs.dimension; i++) {
            tmp[i] = rhs.coordinates[i];
        }
...
void MultiDimensionalPoint::setCoordinate(int x, double value) {
    if (x < 0 || x >= this->dimension) {
        std::stringstream ss;
        ss << "ERR:: " << x << " is out of range. Valid range: [0, " << this->dimension - 1 << "]" << std::endl;
...
double MultiDimensionalPoint::euclideanDistance(const MultiDimensionalPoint& rhs) const {
    // their dimensions must match
   if (this->dimension != rhs.dimension) {
       std::stringstream ss;
       ss << "ERR: Dimensions don't match: " << this->dimension << " != " << rhs.dimension << std::endl;
       throw std::out_of_range(ss.str());
   }
   double total = 0;
   for (int i = 0; i < this->dimension; i++) {
       double tmp_diff = pow(rhs.coordinates[i], 2) - pow(this->coordinates[i], 2);
...
double MultiDimensionalPoint::manhattanDistance(const MultiDimensionalPoint& rhs) const {
    // their dimensions must match
   if (this->dimension != rhs.dimension) {
       std::stringstream ss;
       ss << "ERR: Dimensions don't match: " << this->dimension << " != " << rhs.dimension << std::endl;
